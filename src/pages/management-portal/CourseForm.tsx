...
export default function CourseForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const categoriesData = await adminApi.categories.getAll();
        ...
        if (isEdit && id) {
          ...
          const lessons = await adminApi.lessons.getByCourse(courseId);
                ...
                ? [{
                  ...
                  lessons: lessons
                    ...
                    .map(lesson => ({
                      ...
                      videoSource: "DRIVE" as const, // Default to DRIVE
                      duration: formatDuration(lesson.duration ? lesson.duration * 60 : 0), // Convert minutes to seconds
  ...
  const onSubmit = async (data: CourseFormData) => {
    ...
    try {
      ...
      // Process the course data for Laravel API
      const coursePayload = {
        category_id: parseInt(data.category),
        title: data.title,
        description: data.description,
        thumbnail_url: data.thumbnailUrl || undefined,
        video_url: data.demoVideoUrl || undefined,
        price: data.isFree ? 0 : (data.price || 0),
        level: data.level.toUpperCase() as 'BEGINNER' | 'INTERMEDIATE' | 'ADVANCED',
        status: 'DRAFT' as const,
      };

      console.log('Course data being sent to API:', coursePayload);

      if (isEdit && id) {
        // Update existing course
        const course = await adminApi.courses.update(parseInt(id), coursePayload);
        courseId = course.id;

        // Delete existing lessons for this course (will be recreated)
        const existingLessons = await adminApi.lessons.getByCourse(courseId);
        for (const lesson of existingLessons) {
          await adminApi.lessons.delete(courseId, lesson.id);
        }
      } else {
        // Create new course
        const newCourse = await adminApi.courses.create(coursePayload);
        courseId = newCourse.id;
        isNewCourse = true;
      }

      // Create lessons
      const lessonsToCreate = data.sections.flatMap((section, sectionIndex) =>
        section.lessons.map((lesson, lessonIndex) => ({
          ...
          video_url: lesson.videoSource === "DRIVE"
            ...
            : lesson.videoUrl,
          duration: Math.floor(parseDuration(lesson.duration) / 60), // Convert seconds to minutes
          order_number: sectionIndex * 1000 + lessonIndex,
          is_preview: lessonIndex === 0 && sectionIndex === 0, // First lesson of first section is preview
          video_source: lesson.videoSource, // Include video source
        }))
      );

      // Create each lesson
      for (const lessonData of lessonsToCreate) {
        await adminApi.lessons.create(courseId, lessonData);
      }
      ...
      // If course was just created and lesson creation failed, delete the course
      if (isNewCourse && courseId) {
        try {
          await adminApi.courses.delete(courseId);
          console.log('Rolled back course creation due to lesson error');
        } catch (deleteError) {
